...
const SUBJECT_STYLE: Record<PolityCategory, { badge: string; back: string; icon: string }> = {
  'Important Articles':        { badge: 'text-indigo-600 border-indigo-500',  back: 'from-indigo-600 to-violet-700',  icon: '📖' },
  'Schedules':                 { badge: 'text-teal-600 border-teal-500',      back: 'from-teal-600 to-cyan-700',       icon: '📋' },
  'Amendments':                { badge: 'text-blue-600 border-blue-500',      back: 'from-blue-600 to-indigo-700',     icon: '✏️' },
  'Landmark Cases':            { badge: 'text-green-600 border-green-500',    back: 'from-green-600 to-emerald-700',   icon: '⚖️' },
  'Constitutional Bodies':     { badge: 'text-red-600 border-red-500',        back: 'from-red-600 to-rose-700',        icon: '🏗️' },
  'Parliament & Executive':    { badge: 'text-violet-600 border-violet-500',  back: 'from-violet-600 to-purple-700',   icon: '🏛️' },
  'Fundamental Rights & DPSP': { badge: 'text-amber-600 border-amber-500',    back: 'from-amber-600 to-orange-700',    icon: '🛡️' },
}

export default function Flashcard() {
  const [deck]             = useState(() => shuffle(polityData))
  ...
  return (
    <section id="pol-flash" className="py-16 bg-brand-900">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">

        {/* Header */}
        <div className="text-center mb-10">
          <p className="text-xs font-bold tracking-widest text-amber-400 uppercase mb-1">Section 03</p>
          <h2 className="text-3xl font-extrabold text-white">Polity Flashcards</h2>
