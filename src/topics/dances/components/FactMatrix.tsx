...
const SUBJECTS: ('all' | DanceCategory)[] = [
  'all', 'Bharatanatyam', 'Kathak', 'Kathakali', 'Odissi', 'Kuchipudi',
...
export default function FactMatrix() {
  const [filterSubject, setFilterSubject] = useState<'all' | DanceCategory>('all')
  const [filterProb, setFilterProb]       = useState<'all' | ExamProb>('all')
  const [search, setSearch]               = useState('')
  const [expanded, setExpanded]           = useState<number | null>(null)
  const [page, setPage]                   = useState(1)

  const studyEntries = useMemo(() => danceData.filter(e => !e.questionType), [])
  ...
  return (
    <section id="dn-matrix" className="py-14 bg-slate-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">

        <div className="mb-6">
          <p className="text-xs font-bold tracking-widest text-fuchsia-600 uppercase mb-1">Section 03</p>
          <h2 className="text-3xl font-extrabold text-brand-900">Dance Fact Matrix</h2>
        ...
        <div className="space-y-3 mb-5">
          ...
          <div className="relative max-w-sm">
            <input
              ...
              className="w-full pl-9 pr-4 py-2 rounded-xl border border-slate-200 bg-white text-sm focus:outline-none focus:ring-2 focus:ring-fuchsia-400"
            />
          </div>
                ...
                {paginated.length === 0 ? (
                  ...
                  <Fragment key={e.id}>
                    <tr
                      ...
                      <td className="py-3 px-4 text-fuchsia-700 font-bold">{highlight(e.answer, search)}</td>
                      <td className="py-3 px-4">
                    ...
                    {expanded === e.id && (
                      <tr>
                        <td colSpan={6} className="px-4 py-3 bg-fuchsia-50/60 animate-fade-slide">
                          <p className="text-sm text-fuchsia-800 leading-relaxed mb-1">
                            <strong>Details:</strong> {e.detail}
                          </p>
                          {e.shortcut && (
                            <p className="font-mnemonic text-xs text-fuchsia-700 italic mt-1">
                              Shortcut: &quot;{e.shortcut}&quot;
                            </p>
                          )}
