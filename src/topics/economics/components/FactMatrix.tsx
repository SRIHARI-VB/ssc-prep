...
const SUBJECTS: ('all' | EconCategory)[] = [
  'all', 'Basic Concepts', 'Indian Economy & Planning', 'Banking & Monetary Policy',
...
export default function FactMatrix() {
  const [filterSubject, setFilterSubject] = useState<'all' | EconCategory>('all')
  ...
  return (
    <section id="econ-matrix" className="py-14 bg-slate-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">

        <div className="mb-6">
          <p className="text-xs font-bold tracking-widest text-cyan-700 uppercase mb-1">Section 03</p>
          <h2 className="text-3xl font-extrabold text-brand-900">Economics Fact Matrix</h2>
        ...
        <div className="space-y-3 mb-5">
          ...
          <div className="relative max-w-sm">
            ...
            <input
              ...
              className="w-full pl-9 pr-4 py-2 rounded-xl border border-slate-200 bg-white text-sm focus:outline-none focus:ring-2 focus:ring-cyan-400"
            />
          </div>
          ...
          <div className="overflow-x-auto">
            <table className="text-left min-w-[1200px] w-full">
              <thead className="bg-slate-50 text-xs font-bold uppercase tracking-wider text-slate-500 border-b border-slate-100">
                <tr>
                  <th className="py-3 px-4 min-w-[130px] whitespace-nowrap">Category</th>
                ...
                {paginated.length === 0 ? (
                  ...
                  <Fragment key={e.id}>
                    <tr
                      ...
                      <td className="py-3 px-4 text-cyan-700 font-bold">{highlight(e.answer, search)}</td>
                      <td className="py-3 px-4">
                    ...
                    {expanded === e.id && (
                      <tr>
                        <td colSpan={6} className="px-4 py-3 bg-cyan-50/60 animate-fade-slide">
                          <p className="text-sm text-cyan-800 leading-relaxed mb-1">
                            <strong>Details:</strong> {e.detail}
                          </p>
                          {e.shortcut && (
                            <p className="font-mnemonic text-xs text-cyan-700 italic mt-1">
                              Shortcut: &quot;{e.shortcut}&quot;
                            </p>
                          )}
                          {e.place && (
            ...
            <div className="flex items-center justify-between px-4 py-3 border-t border-slate-100 bg-slate-50">
              ...
              <div className="flex items-center gap-1.5">
                <button onClick={() => { setPage(p => p - 1); setExpanded(null) }} disabled={safePage === 1}
                  className="px-3 py-1.5 text-xs font-semibold rounded-lg border bg-white border-slate-200 hover:border-cyan-400 text-slate-600 disabled:opacity-40 disabled:cursor-not-allowed transition-all">
                  Prev
                </button>
