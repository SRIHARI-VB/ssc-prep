...
const SUBJECT_COLORS: Record<Subject, string> = {
  'Physics':        'rgba(99,102,241,0.8)',
  'Chemistry':      'rgba(20,184,166,0.8)',
  'Biology':        'rgba(34,197,94,0.8)',
  'Space & Defense':'rgba(244,63,94,0.8)',
  'Technology':     'rgba(251,191,36,0.8)',
}
...
export default function Analytics() {
  const subjectCounts = useMemo(() => {
    const counts: Record<Subject, number> = { Physics: 0, Chemistry: 0, Biology: 0, 'Space & Defense': 0, Technology: 0 }
    sciData.forEach(e => { counts[e.subject]++ })
    return counts
  }, [])

  const probCounts = useMemo(() => {
    const counts: Record<string, number> = { Hot: 0, High: 0, Confirmed: 0, Recurring: 0 }
    sciData.forEach(e => { counts[e.examProb]++ })
    return counts
  }, [])

  const subjects = Object.keys(subjectCounts) as Subject[]
  ...
  const confirmedCount = sciData.filter(e => e.examProb === 'Confirmed').length

  return (
    <section id="st-overview" className="py-14 bg-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">

        {/* Header */}
        <div className="mb-8">
          <p className="text-xs font-bold tracking-widest text-teal-600 uppercase mb-1">Section 01</p>
          ...
          <p className="mt-2 text-slate-500 text-sm max-w-2xl">
            {sciData.length} facts across {subjects.length} subjects — SSC CGL PYQs (2022–2024) + Current Affairs (2024–25).
          </p>
        </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-10">
          {[
            { n: sciData.length, label: 'Total Facts', color: 'bg-teal-50 border-teal-200', textColor: 'text-teal-700' },
            ...
            { n: subjects.length, label: 'Subjects', color: 'bg-violet-50 border-violet-200', textColor: 'text-violet-700' },
          ].map(s => (
