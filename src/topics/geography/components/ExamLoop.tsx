...
import { geoData, type GeoCategory, type ExamProb, type GeoEntry } from '../data'
...
const SUBJECT_ICON: Record<GeoCategory, string> = {
  'Indian Rivers': '🏞️', 'Mountains & Passes': '🏔️', 'Soil & Climate': '🌦️',
  'National Parks & Wildlife': '🐅', 'Agriculture & Crops': '🌾', 'Minerals & Resources': '💎',
  'Dams & Irrigation': '🌊', 'World Geography': '🌍', 'Straits & Waterways': '🚢',
  'Transport & Industry': '🚂', 'Population & Census': '📊', 'Current Affairs (Geo)': '📰',
}
const SUBJECT_COLOR: Record<GeoCategory, string> = {
  'Indian Rivers':             'border-blue-400 bg-blue-50 text-blue-700',
  'Mountains & Passes':        'border-violet-400 bg-violet-50 text-violet-700',
  'Soil & Climate':            'border-yellow-400 bg-yellow-50 text-yellow-700',
  'National Parks & Wildlife':  'border-green-400 bg-green-50 text-green-700',
  'Agriculture & Crops':       'border-lime-400 bg-lime-50 text-lime-700',
  'Minerals & Resources':      'border-orange-400 bg-orange-50 text-orange-700',
  'Dams & Irrigation':         'border-cyan-400 bg-cyan-50 text-cyan-700',
  'World Geography':           'border-indigo-400 bg-indigo-50 text-indigo-700',
  'Straits & Waterways':       'border-sky-400 bg-sky-50 text-sky-700',
  'Transport & Industry':      'border-purple-400 bg-purple-50 text-purple-700',
  'Population & Census':       'border-pink-400 bg-pink-50 text-pink-700',
  'Current Affairs (Geo)':     'border-red-400 bg-red-50 text-red-700',
}
...
function pickFrom(pool: GeoEntry[]): GeoEntry {
  return pool[Math.floor(Math.random() * pool.length)]
}

function buildPool(sub: 'all' | GeoCategory, prob: 'all' | ExamProb): GeoEntry[] {
  const pool = geoData.filter(e => {
    if (sub  !== 'all' && e.category !== sub)  return false
    if (prob !== 'all' && e.examProb !== prob) return false
    return true
  })
  return pool.length > 0 ? pool : geoData
}

function getOptions(correct: GeoEntry, pool: GeoEntry[], count = 4): string[] {
  const distractors = shuffle(
    (pool.length >= count ? pool : geoData)
...
export default function ExamLoop() {
  const [filterSub,  setFilterSub]  = useState<'all' | GeoCategory>('all')
  ...
  const [state, setState] = useState<LoopState>(() => {
    const card = pickFrom(geoData)
    return { card, options: getOptions(card, geoData), phase: 'question', score: 0, total: 0 }
  })
  const [selected, setSelected] = useState<string | null>(null)
  const [shake, setShake] = useState(false)

  const resetQuiz = useCallback((pool: GeoEntry[]) => {
    const card = pickFrom(pool)
    setSelected(null)
    setState({ card, options: getOptions(card, pool), phase: 'question', score: 0, total: 0 })
  }, [])

  const handleFilter = useCallback((sub: 'all' | GeoCategory) => {
    setFilterSub(sub)
    resetQuiz(buildPool(sub, filterProb))
  }, [filterProb, resetQuiz])

  const handleProbFilter = useCallback((prob: 'all' | ExamProb) => {
    setFilterProb(prob)
    resetQuiz(buildPool(filterSub, prob))
  }, [filterSub, resetQuiz])
  ...
  return (
    <section id="geo-loop" className="py-16 bg-white">
        ...
        <div className="bg-slate-50 rounded-2xl border border-slate-200 p-4 mb-8 space-y-3">
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              {SUBJECTS.map(s => {
                const count = s === 'all' ? geoData.length : geoData.filter(e => e.category === s).length
                return (
                  <button key={s} onClick={() => handleFilter(s)}
                    ...
                    {s === 'all' ? `All (${count})` : `${SUBJECT_ICON[s as GeoCategory]} ${(s as string).split(' ')[0]}... (${count})`}
                  </button>
                )
              })}
            </div>
          ...
          <div className="border-t border-slate-200 pt-3">
            ...
            <div className="flex flex-wrap gap-2">
              {PROBS.map(p => {
                const count = p === 'all' ? geoData.length : geoData.filter(e => e.examProb === p).length
        ...
        <div className="max-w-2xl mx-auto">
          {(filterSub !== 'all' || filterProb !== 'all') && (
            <div className="flex flex-wrap gap-2 mb-4">
              {filterSub !== 'all' && (
                <span className={`text-xs font-bold px-3 py-1 rounded-full border ${SUBJECT_COLOR[filterSub as GeoCategory]}`}>
                  {SUBJECT_ICON[filterSub as GeoCategory]} Drilling: {filterSub}
                </span>
              )}
              {filterProb !== 'all' && (
