...
import { miData, type MICategory, type ExamProb } from '../data'
...
const CATEGORY_DEF: CategoryMap = {
  'String Instruments (Tata)':             { icon: '\uD83C\uDFB8', color: 'border-rose-400 bg-rose-50 text-rose-700' },
  'Wind Instruments (Sushira)':            { icon: '\uD83C\uDFB5', color: 'border-orange-400 bg-orange-50 text-orange-700' },
  'Percussion - Membranous (Avanaddha)':   { icon: '\uD83E\uDD41', color: 'border-violet-400 bg-violet-50 text-violet-700' },
  'Percussion - Solid (Ghana)':            { icon: '\uD83D\uDD14', color: 'border-green-400 bg-green-50 text-green-700' },
  'Hindustani Classical':                  { icon: '\uD83C\uDFB6', color: 'border-blue-400 bg-blue-50 text-blue-700' },
  'Carnatic Classical':                    { icon: '\uD83C\uDFB9', color: 'border-pink-400 bg-pink-50 text-pink-700' },
  'Famous Musicians & Awards':             { icon: '\uD83C\uDFC6', color: 'border-yellow-400 bg-yellow-50 text-yellow-700' },
  'Current Affairs & Awards':              { icon: '\uD83D\uDCF0', color: 'border-red-400 bg-red-50 text-red-700' },
}

const ACCENT: AccentPalette = {
  chipActive: 'bg-rose-600 text-white border-rose-600',
  optHover:   'hover:border-rose-400 hover:bg-rose-50 hover:text-rose-700',
  bar:        'bg-rose-400',
...
export default function ExamLoop() {
  const [filterSub,  setFilterSub]  = useState<'all' | MICategory | 'SSC Format'>('all')
  const [filterProb, setFilterProb] = useState<'all' | ExamProb>('all')

  const filtered = useMemo<EngineQuestion[]>(() => {
    const list = miData.filter(q => {
    ...
    return (list.length > 0 ? list : miData) as any as EngineQuestion[]
  }, [filterSub, filterProb])

  const sscCount = miData.filter(q => !!q.questionType).length
  ...
  return (
    <section className="py-16 bg-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {/* Section header */}
        <div className="mb-8">
          <p className="text-xs font-bold tracking-widest text-rose-600 uppercase mb-1">Section 06</p>
          <h2 className="text-3xl font-extrabold text-brand-900">Music &amp; Instruments -- Exam Loop</h2>
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              <button onClick={() => setFilterSub('all')}
                className={`px-3 py-1.5 rounded-full text-xs font-bold border transition-all ${filterSub === 'all' ? ACCENT.chipActive : 'bg-white text-slate-600 border-slate-200 hover:border-rose-400'}`}>
                All ({miData.length})
              </button>
              ...
              {CATEGORIES.map(s => (
                <button key={s} onClick={() => setFilterSub(s)}
                  className={`px-3 py-1.5 rounded-full text-xs font-bold border transition-all ${filterSub === s ? ACCENT.chipActive : 'bg-white text-slate-600 border-slate-200 hover:border-rose-400'}`}>
                  {CATEGORY_DEF[s].icon} {s.split(' ')[0]}... ({miData.filter(q => q.category === s).length})
                </button>
              ))}
            </div>
          </div>
            ...
            <div className="flex flex-wrap gap-2">
              ...
              {PROBS.map(p => (
                <button key={p} onClick={() => setFilterProb(p)}
                  ...
                  {p} ({miData.filter(q => q.examProb === p).length})
                </button>
              ))}
            </div>
          </div>
        ...
        {(filterSub !== 'all' || filterProb !== 'all') && (
          <div className="flex flex-wrap gap-2 mb-4 max-w-2xl mx-auto">
            ...
            {filterSub !== 'all' && filterSub !== 'SSC Format' && (
              <span className={`text-xs font-bold px-3 py-1 rounded-full border ${CATEGORY_DEF[filterSub as MICategory]?.color ?? ''}`}>
                {CATEGORY_DEF[filterSub as MICategory]?.icon} Drilling: {filterSub}
              </span>
            )}
            {filterProb !== 'all' && (
