...
const SUBJECT_STYLE: Record<MICategory, { badge: string; back: string; icon: string }> = {
  'String Instruments (Tata)':             { badge: 'text-rose-600 border-rose-500',    back: 'from-rose-600 to-pink-700',     icon: '\uD83C\uDFB8' },
  'Wind Instruments (Sushira)':            { badge: 'text-orange-600 border-orange-500', back: 'from-orange-600 to-amber-700',  icon: '\uD83C\uDFB5' },
  'Percussion - Membranous (Avanaddha)':   { badge: 'text-violet-600 border-violet-500', back: 'from-violet-600 to-purple-700', icon: '\uD83E\uDD41' },
  'Percussion - Solid (Ghana)':            { badge: 'text-green-600 border-green-500',  back: 'from-green-600 to-emerald-700',  icon: '\uD83D\uDD14' },
  'Hindustani Classical':                  { badge: 'text-blue-600 border-blue-500',    back: 'from-blue-600 to-indigo-700',    icon: '\uD83C\uDFB6' },
  'Carnatic Classical':                    { badge: 'text-pink-600 border-pink-500',    back: 'from-pink-600 to-rose-700',      icon: '\uD83C\uDFB9' },
  'Famous Musicians & Awards':             { badge: 'text-yellow-600 border-yellow-500', back: 'from-yellow-600 to-amber-700',  icon: '\uD83C\uDFC6' },
  'Current Affairs & Awards':              { badge: 'text-red-600 border-red-500',      back: 'from-red-600 to-rose-700',       icon: '\uD83D\uDCF0' },
  'Folk Music':                            { badge: 'text-purple-600 border-purple-500', back: 'from-purple-600 to-violet-700', icon: '\uD83C\uDFB6' },
}

export default function Flashcard() {
  const [filterCat, setFilterCat] = useState<'all' | MICategory>('all')

  const [deck, setDeck]      = useState(() => shuffle(miData))
  const [index, setIndex]    = useState(0)
  const [flipped, setFlip]   = useState(false)
  const [seen, setSeen]      = useState(0)

  const categories = useMemo(() => {
    const cats = [...new Set(miData.map(e => e.category))] as MICategory[]
    return ['all' as const, ...cats]
  }, [])

  const handleCatChange = useCallback((c: 'all' | MICategory) => {
    setFilterCat(c)
    const newPool = c === 'all' ? miData : miData.filter(e => e.category === c)
  ...
  const style = card ? SUBJECT_STYLE[card.category] : SUBJECT_STYLE['String Instruments (Tata)']
  ...
  return (
    <section id="mi-flash" className="py-16 bg-brand-900">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">

        <div className="text-center mb-10">
          <p className="text-xs font-bold tracking-widest text-rose-400 uppercase mb-1">Section 05</p>
          <h2 className="text-3xl font-extrabold text-white">Music &amp; Instruments Flashcards</h2>
          ...
          <div className="flex flex-wrap justify-center gap-2 mt-4">
            {categories.map(c => {
              const count = c === 'all' ? miData.length : miData.filter(e => e.category === c).length
              return (
                ...
                <div className="flex-1 flex flex-col items-center justify-center text-center px-2 gap-2 overflow-y-auto">
                  ...
                  {card.shortcut && (
                    <div className="mt-2 bg-white/10 rounded-xl px-4 py-2 border border-white/20 w-full">
                      <p className="font-mnemonic text-xs text-rose-300 italic">&quot;{card.shortcut}&quot;</p>
                    </div>
                  )}
                </div>
                <button
