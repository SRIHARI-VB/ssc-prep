...
const STYLE: Record<HistoryCategory, { badge: string; back: string; icon: string }> = {
  'Ancient India':     { badge: 'text-amber-600 border-amber-500',   back: 'from-amber-600 to-orange-700',  icon: '🏛️' },
  'Medieval India':    { badge: 'text-orange-600 border-orange-500', back: 'from-orange-600 to-red-700',    icon: '🕌' },
  'Modern India':      { badge: 'text-blue-600 border-blue-500',    back: 'from-blue-600 to-indigo-700',   icon: '🏗️' },
  'Freedom Struggle':  { badge: 'text-green-600 border-green-500',  back: 'from-green-600 to-emerald-700', icon: '🇮🇳' },
  'Reform Movements':  { badge: 'text-violet-600 border-violet-500',back: 'from-violet-600 to-purple-700', icon: '✊' },
  'Art & Culture':     { badge: 'text-pink-600 border-pink-500',    back: 'from-pink-600 to-rose-700',     icon: '🎨' },
  'Battles & Wars':    { badge: 'text-red-600 border-red-500',      back: 'from-red-600 to-rose-700',      icon: '⚔️' },
}
...
export default function Flashcard() {
  ...
  const categories = useMemo(() => {
    const cats = [...new Set(studyEntries.map(e => e.category))] as HistoryCategory[]
    return ['all' as const, ...cats]
  }, [studyEntries])

  const handleCatChange = useCallback((c: 'all' | HistoryCategory) => {
    setFilterCat(c)
  ...
  const style = card ? STYLE[card.category] : STYLE['Ancient India']
  ...
  return (
    <section id="hist-flash" className="py-16 bg-brand-900">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">

        <div className="text-center mb-10">
          <p className="text-xs font-bold tracking-widest text-amber-400 uppercase mb-1">Section 05</p>
          <h2 className="text-3xl font-extrabold text-white">History Flashcards</h2>
                ...
                <div className="flex items-center justify-between mb-4">
                  ...
                  {card.year && (
                    <span className="text-xs text-slate-400 font-mono">
                      {card.year < 0 ? `${Math.abs(card.year)} BCE` : `${card.year} CE`}
                    </span>
                  )}
                </div>
