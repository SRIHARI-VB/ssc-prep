...
const SUBJECTS: ('all' | SportsCategory)[] = [
  'all', 'Cricket', 'Olympics & CWG', 'Hockey', 'Football',
...
export default function FactMatrix() {
  const [filterSubject, setFilterSubject] = useState<'all' | SportsCategory>('all')
  const [filterProb, setFilterProb]       = useState<'all' | ExamProb>('all')
  const [search, setSearch]               = useState('')
  const [expanded, setExpanded]           = useState<number | null>(null)
  const [page, setPage]                   = useState(1)

  const studyEntries = useMemo(() => sportsData.filter(e => !e.questionType), [])
  ...
  return (
    <section id="sp-matrix" className="py-14 bg-slate-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">

        <div className="mb-6">
          <p className="text-xs font-bold tracking-widest text-orange-600 uppercase mb-1">Section 03</p>
          <h2 className="text-3xl font-extrabold text-brand-900">Sports Fact Matrix</h2>
        ...
        <div className="space-y-3 mb-5">
          ...
          <div className="relative max-w-sm">
            <input
              ...
              className="w-full pl-9 pr-4 py-2 rounded-xl border border-slate-200 bg-white text-sm focus:outline-none focus:ring-2 focus:ring-orange-400"
            />
          </div>
                ...
                {paginated.length === 0 ? (
                  ...
                  <Fragment key={e.id}>
                    <tr
                      ...
                      <td className="py-3 px-4 text-orange-700 font-bold">{highlight(e.answer, search)}</td>
                      <td className="py-3 px-4">
                    ...
                    {expanded === e.id && (
                      <tr>
                        <td colSpan={6} className="px-4 py-3 bg-orange-50/60 animate-fade-slide">
                          <p className="text-sm text-orange-800 leading-relaxed mb-1">
                            <strong>Details:</strong> {e.detail}
                          </p>
                          {e.shortcut && (
                            <p className="font-mnemonic text-xs text-orange-700 italic mt-1">
                              Shortcut: &quot;{e.shortcut}&quot;
                            </p>
                          )}
