...
import { sportsData, type SportsCategory, type ExamProb } from '../data'
...
const CATEGORY_DEF: CategoryMap = {
  'Cricket':                { icon: '\u{1F3CF}', color: 'border-orange-400 bg-orange-50 text-orange-700' },
  'Olympics & CWG':         { icon: '\u{1F3C5}', color: 'border-red-400 bg-red-50 text-red-700' },
  'Hockey':                 { icon: '\u{1F3D1}', color: 'border-blue-400 bg-blue-50 text-blue-700' },
  'Football':               { icon: '\u26BD', color: 'border-green-400 bg-green-50 text-green-700' },
  'Tennis & Badminton':     { icon: '\u{1F3F8}', color: 'border-violet-400 bg-violet-50 text-violet-700' },
  'Athletics & Boxing':     { icon: '\u{1F94A}', color: 'border-pink-400 bg-pink-50 text-pink-700' },
  'Awards & Honours':       { icon: '\u{1F3C6}', color: 'border-amber-400 bg-amber-50 text-amber-700' },
  'Tournaments & Trophies': { icon: '\u{1F947}', color: 'border-cyan-400 bg-cyan-50 text-cyan-700' },
  'Rules & Players':        { icon: '\u{1F4CB}', color: 'border-indigo-400 bg-indigo-50 text-indigo-700' },
  'Venues & Events':        { icon: '\u{1F3DF}\uFE0F', color: 'border-lime-400 bg-lime-50 text-lime-700' },
  'Current Affairs 2024-26':{ icon: '\u{1F4F0}', color: 'border-rose-400 bg-rose-50 text-rose-700' },
}

const ACCENT: AccentPalette = {
  chipActive: 'bg-orange-600 text-white border-orange-600',
  optHover:   'hover:border-orange-400 hover:bg-orange-50 hover:text-orange-700',
  bar:        'bg-orange-400',
  scoreText:  'text-orange-600',
  nextBtn:    'bg-orange-600 hover:bg-orange-500 text-white',
  text:       'text-orange-600',
}

const CATEGORIES: SportsCategory[] = [
...
export default function ExamLoop() {
  const [filterSub,  setFilterSub]  = useState<'all' | SportsCategory | 'SSC Format'>('all')
  const [filterProb, setFilterProb] = useState<'all' | ExamProb>('all')

  const filtered = useMemo<EngineQuestion[]>(() => {
    const list = sportsData.filter(q => {
    ...
    return (list.length > 0 ? list : sportsData) as any as EngineQuestion[]
  }, [filterSub, filterProb])

  const sscCount = sportsData.filter(q => !!q.questionType).length
  ...
  return (
    <section className="py-16 bg-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {/* Section header */}
        <div className="mb-8">
          <p className="text-xs font-bold tracking-widest text-orange-600 uppercase mb-1">Section 06</p>
          <h2 className="text-3xl font-extrabold text-brand-900">Sports -- Exam Loop</h2>
          <p className="mt-1 text-slate-500 text-sm">
            Every question has <strong>4 hand-picked, topically relevant options</strong>.
            Use <span className="text-violet-600 font-bold">{'\u{1F4CB}'} SSC Format</span> to drill
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              <button onClick={() => setFilterSub('all')}
                className={`px-3 py-1.5 rounded-full text-xs font-bold border transition-all ${filterSub === 'all' ? ACCENT.chipActive : 'bg-white text-slate-600 border-slate-200 hover:border-orange-400'}`}>
                {'\u{1F30D}'} All ({sportsData.length})
              </button>
              <button onClick={() => setFilterSub('SSC Format')}
                ...
                {'\u{1F4CB}'} SSC Format ({sscCount})
              </button>
              {CATEGORIES.map(s => (
                <button key={s} onClick={() => setFilterSub(s)}
                  className={`px-3 py-1.5 rounded-full text-xs font-bold border transition-all ${filterSub === s ? ACCENT.chipActive : 'bg-white text-slate-600 border-slate-200 hover:border-orange-400'}`}>
                  {CATEGORY_DEF[s].icon} {s} ({sportsData.filter(q => q.category === s).length})
                </button>
              ))}
            </div>
          </div>
          ...
          <div className="border-t border-slate-200 pt-3">
            ...
            <div className="flex flex-wrap gap-2">
              <button onClick={() => setFilterProb('all')}
                ...
                All Priorities ({sportsData.length})
              </button>
              {PROBS.map(p => (
                <button key={p} onClick={() => setFilterProb(p)}
                  ...
                  {p} ({sportsData.filter(q => q.examProb === p).length})
                </button>
              ))}
            </div>
          </div>
          ...
          <div className="flex flex-wrap gap-2 mb-4 max-w-2xl mx-auto">
            {filterSub === 'SSC Format' && (
              <span className="text-xs font-bold px-3 py-1 rounded-full border bg-violet-100 text-violet-700 border-violet-300">
                {'\u{1F4CB}'} SSC Format only
              </span>
            )}
            {filterSub !== 'all' && filterSub !== 'SSC Format' && (
              <span className={`text-xs font-bold px-3 py-1 rounded-full border ${CATEGORY_DEF[filterSub as SportsCategory]?.color ?? ''}`}>
                {CATEGORY_DEF[filterSub as SportsCategory]?.icon} Drilling: {filterSub}
              </span>
            )}
            {filterProb !== 'all' && (
