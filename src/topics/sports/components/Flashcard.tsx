...
const SUBJECT_STYLE: Record<SportsCategory, { badge: string; back: string; icon: string }> = {
  'Cricket':                { badge: 'text-orange-600 border-orange-500',  back: 'from-orange-600 to-amber-700',    icon: '\u{1F3CF}' },
  'Olympics & CWG':         { badge: 'text-red-600 border-red-500',        back: 'from-red-600 to-rose-700',        icon: '\u{1F3C5}' },
  'Hockey':                 { badge: 'text-blue-600 border-blue-500',      back: 'from-blue-600 to-indigo-700',     icon: '\u{1F3D1}' },
  'Football':               { badge: 'text-green-600 border-green-500',    back: 'from-green-600 to-emerald-700',   icon: '\u26BD' },
  'Tennis & Badminton':     { badge: 'text-violet-600 border-violet-500',  back: 'from-violet-600 to-purple-700',   icon: '\u{1F3F8}' },
  'Athletics & Boxing':     { badge: 'text-pink-600 border-pink-500',      back: 'from-pink-600 to-rose-700',       icon: '\u{1F94A}' },
  'Awards & Honours':       { badge: 'text-amber-600 border-amber-500',    back: 'from-amber-600 to-yellow-700',    icon: '\u{1F3C6}' },
  'Tournaments & Trophies': { badge: 'text-cyan-600 border-cyan-500',      back: 'from-cyan-600 to-blue-700',       icon: '\u{1F947}' },
  'Rules & Players':        { badge: 'text-indigo-600 border-indigo-500',  back: 'from-indigo-600 to-violet-700',   icon: '\u{1F4CB}' },
  'Venues & Events':        { badge: 'text-lime-600 border-lime-500',      back: 'from-lime-600 to-green-700',      icon: '\u{1F3DF}\uFE0F' },
  'Current Affairs 2024-26':{ badge: 'text-rose-600 border-rose-500',      back: 'from-rose-600 to-red-700',        icon: '\u{1F4F0}' },
}

export default function Flashcard() {
  const [filterCat, setFilterCat] = useState<'all' | SportsCategory>('all')

  const [deck, setDeck]      = useState(() => shuffle(sportsData))
  const [index, setIndex]    = useState(0)
  const [flipped, setFlip]   = useState(false)
  const [seen, setSeen]      = useState(0)

  const categories = useMemo(() => {
    const cats = [...new Set(sportsData.map(e => e.category))] as SportsCategory[]
    return ['all' as const, ...cats]
  }, [])

  const handleCatChange = useCallback((c: 'all' | SportsCategory) => {
    setFilterCat(c)
    const newPool = c === 'all' ? sportsData : sportsData.filter(e => e.category === c)
  ...
  const style = card ? SUBJECT_STYLE[card.category] : SUBJECT_STYLE['Cricket']
  ...
  return (
    <section id="sp-flash" className="py-16 bg-brand-900">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">

        <div className="text-center mb-10">
          <p className="text-xs font-bold tracking-widest text-orange-400 uppercase mb-1">Section 05</p>
          <h2 className="text-3xl font-extrabold text-white">Sports Flashcards</h2>
          ...
          <div className="flex flex-wrap justify-center gap-2 mt-4">
            {categories.map(c => {
              const count = c === 'all' ? sportsData.length : sportsData.filter(e => e.category === c).length
              return (
                ...
                <div className="flex-1 flex flex-col items-center justify-center text-center px-2 gap-2 overflow-y-auto">
                  ...
                  {card.shortcut && (
                    <div className="mt-2 bg-white/10 rounded-xl px-4 py-2 border border-white/20 w-full">
                      <p className="font-mnemonic text-xs text-orange-300 italic">&quot;{card.shortcut}&quot;</p>
                    </div>
                  )}
                </div>
                <button
